...
def as_clusters(df):
    ...
    for customer_id, transactions in tqdm(df.groupby('customer_id')):
        ...
        for cluster_id, cluster in transactions.groupby('cluster_id'):
            ...
            work_distance = distance(np.array([work_add_lat, work_add_lon]), np.array([cluster_median.values]))[0]
            ...
            mcc_histogram = cluster.mcc.astype(CategoricalDtype(categories=mcc_whitelist)).value_counts(normalize=True, dropna=False)
            ...
            features = {
                ...
                'is_close': int(work_distance <= 0.02),
                'area': area,
